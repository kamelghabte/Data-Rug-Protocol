// ════════════════════════════════════════════════════════════════
//  ESSAOUIRA ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 08
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInESSAOUIRA()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInESSAOUIRA() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInESSAOUIRA() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // RIAH — Carreau plein, vent
    rect(x, y, w, h);
  } else if (type === 1) {
    // SHABAK — Bandes verticales, filet
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // ARAAR — Damier, bois de thuya
    rect(x, y, w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);
  } else if (type === 3) {
    // GNAWI — Losange, rythme sacré
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // BORJ — Hélice, bastion
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(10, 10, 10), 0.45));
...
function drawMuseumFrame() {
  ...
  fill(isNightInESSAOUIRA() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInESSAOUIRA() ? "الصويرة بالليل" : "زليج الصويرة", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInESSAOUIRA() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("08", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"رياح",   name:"RIAH",   ref:"VENT · SNAKE"    },
    { type:1, ar:"شبك",   name:"SHABAK",   ref:"FILET · DIAG"    },
    { type:2, ar:"عرعار",   name:"ARAAR",   ref:"THUYA · REVERSE"    },
    { type:3, ar:"كناوي",   name:"GNAWI",   ref:"SACRE · SKIP"    },
    { type:4, ar:"برج",   name:"BORJ",   ref:"BASTION · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInESSAOUIRA();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("ESSAOUIRA_MOGADOR // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 08 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: ESSAOUIRA, MAROC", startX, startY);
