// ════════════════════════════════════════════════════════════════
//  ELJADIDA ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 12
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInELJADIDA()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInELJADIDA() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInELJADIDA() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // SAHRIIJ — Carreau plein, citerne
    rect(x, y, w, h);
  } else if (type === 1) {
    // BORJ — Bandes verticales, bastion
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // MIRAYA — Moitié haute, reflet d'eau
    rect(x, y, w, h/2);
  } else if (type === 3) {
    // QUBBA — Losange, voûte
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // ANKAA — Hélice, phénix portugais
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(10, 10, 14), 0.45));
...
function drawMuseumFrame() {
  ...
  fill(isNightInELJADIDA() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInELJADIDA() ? "الجديدة بالليل" : "زليج الجديدة", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInELJADIDA() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("12", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"صهريج",   name:"SAHRIIJ",   ref:"CITERNE · SNAKE"    },
    { type:1, ar:"برج",   name:"BORJ",   ref:"BASTION · DIAG"    },
    { type:2, ar:"مرآية",   name:"MIRAYA",   ref:"REFLET · REVERSE"    },
    { type:3, ar:"قبّة",   name:"QUBBA",   ref:"VOUTE · SKIP"    },
    { type:4, ar:"عنقاء",   name:"ANKAA",   ref:"PHENIX · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInELJADIDA();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("ELJADIDA_MAZAGAN // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 12 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: EL JADIDA, MAROC", startX, startY);
