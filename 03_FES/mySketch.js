// ════════════════════════════════════════════════════════════════
//  FES ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 03
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInFES()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInFES() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInFES() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // ZILLIJ — Carreau plein, tuile de base
    rect(x, y, w, h);
  } else if (type === 1) {
    // KHATT — Bandes diagonales fines
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // CHOUARA — Cercle dans carré, cuves tanneries
    rect(x, y, w, h);
    fill(lerpColor(color(col), color(10, 10, 10), 0.5));
    ellipse(cx, cy, w*0.7, h*0.7);
  } else if (type === 3) {
    // BJAMAT — Losange, pilier mosquée
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // NAJMA — Hélice étoile, Karaouiyine
...
function drawMuseumFrame() {
  ...
  fill(isNightInFES() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInFES() ? "فاس بالليل" : "زليج فاس", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInFES() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("03", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"زليج",   name:"ZILLIJ",   ref:"BASE · SNAKE"    },
    { type:1, ar:"خط",   name:"KHATT",   ref:"TRAIT · DIAG"    },
    { type:2, ar:"شوّارة",   name:"CHOUARA",   ref:"CUVES · REVERSE"    },
    { type:3, ar:"بجمات",   name:"BJAMAT",   ref:"PILIER · SKIP"    },
    { type:4, ar:"نجمة",   name:"NAJMA",   ref:"ETOILE · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInFES();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("FES_MEDINA // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 03 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: FES, MAROC", startX, startY);
