// ════════════════════════════════════════════════════════════════
//  MARRAKECH ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 04
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInMARRAKECH()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInMARRAKECH() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInMARRAKECH() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // TOUB — Carreau plein, brique ocre
    rect(x, y, w, h);
  } else if (type === 1) {
    // CHEMS — Bandes verticales, chaleur
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // KOUTOUBIA — Damier, ombre du minaret
    rect(x, y, w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);
  } else if (type === 3) {
    // JNANE — Losange, palmeraie
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // ZOUAK — Hélice moucharabieh
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(44, 24, 16), 0.45));
...
function drawMuseumFrame() {
  ...
  fill(isNightInMARRAKECH() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInMARRAKECH() ? "مراكش بالليل" : "زليج مراكش", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInMARRAKECH() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("04", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"طوب",   name:"TOUB",   ref:"BRIQUE · SNAKE"    },
    { type:1, ar:"شمس",   name:"CHEMS",   ref:"SOLEIL · DIAG"    },
    { type:2, ar:"كتبية",   name:"KOUTOUBIA",   ref:"MINARET · REVERSE"    },
    { type:3, ar:"جنان",   name:"JNANE",   ref:"PALME · SKIP"    },
    { type:4, ar:"زواق",   name:"ZOUAK",   ref:"MOUCHARABY · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInMARRAKECH();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("MARRAKECH_OCHRE // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 04 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: MARRAKECH, MAROC", startX, startY);
