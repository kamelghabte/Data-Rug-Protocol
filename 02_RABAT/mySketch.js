// ════════════════════════════════════════════════════════════════
//  RABAT ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 02
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInRABAT()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInRABAT() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInRABAT() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // BOUSTANE — Carreau plein vert, jardin
    rect(x, y, w, h);
  } else if (type === 1) {
    // QASR — Bandes horizontales, palais
    let sh = h/3;
    for (let i = 0; i < 3; i += 2) rect(x, y + i*sh, w, sh);
  } else if (type === 2) {
    // BOUREGREG — Damier, rive double
    rect(x, y, w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);
  } else if (type === 3) {
    // HASSAN — Losange vertical, minaret
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // RIHANE — Croix, jardin andalou
    rect(x + w*0.35, y, w*0.3, h);
    fill(lerpColor(color(col), color(8, 20, 14), 0.4));
...
function drawMuseumFrame() {
  ...
  fill(isNightInRABAT() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInRABAT() ? "الرباط بالليل" : "زليج الأوداية", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInRABAT() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("02", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"بستان",   name:"BOUSTANE",   ref:"BASE · SNAKE"    },
    { type:1, ar:"قصر",   name:"QASR",   ref:"VERT · DIAG"    },
    { type:2, ar:"بورقراق",   name:"BOUREGREG",   ref:"FLEUVE · REVERSE"    },
    { type:3, ar:"حسّان",   name:"HASSAN",   ref:"MINARET · SKIP"    },
    { type:4, ar:"ريحان",   name:"RIHANE",   ref:"JARDIN · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInRABAT();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("RABAT_OUDAYAS // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 02 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: RABAT, MAROC", startX, startY);
