// ════════════════════════════════════════════════════════════════
//  TETOUAN ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 06
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInTETOUAN()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInTETOUAN() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInTETOUAN() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // BAYDA — Carreau plein blanc
    rect(x, y, w, h);
  } else if (type === 1) {
    // BAWABA — Bandes verticales, porte verte
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // TARRAZ — Damier, broderie textile
    rect(x, y, w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);
  } else if (type === 3) {
    // SAQIYA — Losange, fontaine
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // WARD — Hélice, fleur andalouse
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(13, 13, 13), 0.4));
...
function drawMuseumFrame() {
  ...
  fill(isNightInTETOUAN() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInTETOUAN() ? "تطوان بالليل" : "زليج تطوان", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInTETOUAN() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("06", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"بيضاء",   name:"BAYDA",   ref:"BLANC · SNAKE"    },
    { type:1, ar:"بوّابة",   name:"BAWABA",   ref:"PORTE · DIAG"    },
    { type:2, ar:"تطريز",   name:"TARRAZ",   ref:"BRODERIE · REVERSE"    },
    { type:3, ar:"ساقية",   name:"SAQIYA",   ref:"EAU · SKIP"    },
    { type:4, ar:"ورد",   name:"WARD",   ref:"FLEUR · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInTETOUAN();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("TETOUAN_ANDALUS // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 06 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: TETOUAN, MAROC", startX, startY);
