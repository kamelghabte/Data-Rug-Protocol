// DATA RUG PROTOCOL // 02 RABAT
// OUDAYAS GREEN CODE — Capitale Verte / Oudayas Signal
...
let humidity = 65;
let wind = 4;
let description = "OUDAYAS SIGNAL...";
...
// --------------------------------------------------
// 3) PALETTE RABAT
// Vert Oudayas · Vert jardin · Ivoire · Or · Noir
// --------------------------------------------------
let palette = {
  oudayas:  "#1A5276",
  jardin:   "#1E8449",
...
function buildTextureLayer() {
  ...
  for (let i = 0; i < width * height * 0.0008; i++) {
    ...
    let y = random(height);
    let a = random(5, 14);
    textureLayer.fill(255, a);
    textureLayer.circle(x, y, random(0.3, 1.0));
  }

  for (let i = 0; i < 80; i++) {
    textureLayer.stroke(30, 180, 80, 8);
    let y = random(height);
...
function draw() {
  ...
  if (pendingExport) {
    ...
    saveCanvas(`DATA_RUG_IFM_02_RABAT_KAMEL_GHABTE_${ts}`, 'png');
    setTimeout(() => {
...
function drawCells() {
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (cell.active) {
        drawRabatMotif(x, y, cellW * 0.86, cellH * 0.86, cell.type, cell.col, cell.accent, cell.rot, energy);
...
function pickColor() {
  if (temp <= 18) {
    return random([palette.oudayas, palette.jardin, palette.pierre]);
  }
  if (temp >= 30) {
    return random([palette.or, palette.ivoire, palette.mousse]);
  }
  return random([palette.jardin, palette.oudayas, palette.or, palette.mousse]);
}

function pickAccent(baseCol) {
  if (baseCol === palette.ivoire || baseCol === palette.or) return palette.oudayas;
  if (baseCol === palette.jardin || baseCol === palette.mousse) return palette.or;
...
function weavePattern(velocity) {
  ...
  let pool = wind > 8 ? [1, 3, 1, 3, 0] : Array.from({length: complexity + 1}, (_, i) => i);
...
// --------------------------------------------------
// 10) MOTIFS RABAT
// Oudayas, jardins, arcs, Tour Hassan — formes pleines
// --------------------------------------------------
function drawRabatMotif(x, y, w, h, type, col, accent, rot, energy) {
  ...
  if (type === 0) {
    // KASBAH — carré plein + carré intérieur tournant
    fill(col);
    ...
    // ARC HASSAN — arche verticale, Tour Hassan
    fill(col);
    beginShape();
    ...
    // JARDIN — losange, forme végétale, respiration
    fill(col);
    beginShape();
    vertex(0, -h * 0.43);
    vertex(w * 0.43, 0);
    ...
    // BOUREGREG — cercle + bandes, fleuve, pont
    fill(col);
...
function drawHUD() {
  ...
  fill(140);
  textSize(10);
  text("RABAT, MAROC // INSTITUT FRANÇAIS DU MAROC", margin, ty);
  ...
  textSize(10);
  text(
    `34.0209N / 6.8416W  //  TEMP ${nf(temp, 1, 1)}°C  HUM ${humidity}%  WIND ${nf(wind, 1, 1)} m/s`,
    margin, ty + 56
  );
  ...
  fill(palette.ivoire);
  textSize(16);
  text("DATA RUG PROTOCOL // 02 RABAT", width * 0.5, ty);

  fill(palette.mousse);
  textSize(9);
  text("OUDAYAS GREEN CODE  —  Capitale Verte / Oudayas Signal", width * 0.5, ty + 28);
  ...
  rectMode(CORNER);
  fill(palette.oudayas);
  rect(sx, legendY, 10, 10);
  fill(130);
  textAlign(LEFT, TOP);
  textSize(9);
  text("≤ 18°C  OCÉANIQUE", sx + 14, legendY);

  fill(palette.jardin);
  rect(sx + sw, legendY, 10, 10);
  fill(130);
  text("19–29°C  JARDIN", sx + sw + 14, legendY);
  ...
  let motifNames = ["0·KASBAH", "1·ARC", "2·JARDIN", "3·FLEUVE"];
  let mstep = 88;
