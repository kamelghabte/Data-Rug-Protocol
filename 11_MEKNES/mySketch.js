// ════════════════════════════════════════════════════════════════
//  MEKNES ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 11
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInMEKNES()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInMEKNES() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInMEKNES() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // ZEITOUN — Carreau plein, olive
    rect(x, y, w, h);
  } else if (type === 1) {
    // HAJAR — Bandes horizontales, pierre taillée
    let sh = h/3;
    for (let i = 0; i < 3; i += 2) rect(x, y + i*sh, w, sh);
  } else if (type === 2) {
    // BAB — Damier, porte monumentale
    rect(x, y, w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);
  } else if (type === 3) {
    // ROUMI — Losange, mosaïque romaine
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // TAWRIK — Hélice, feuillage
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(10, 10, 4), 0.45));
...
function drawMuseumFrame() {
  ...
  fill(isNightInMEKNES() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInMEKNES() ? "مكناس بالليل" : "زليج مكناس", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInMEKNES() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("11", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"زيتون",   name:"ZEITOUN",   ref:"OLIVE · SNAKE"    },
    { type:1, ar:"حجر",   name:"HAJAR",   ref:"PIERRE · DIAG"    },
    { type:2, ar:"باب",   name:"BAB",   ref:"PORTE · REVERSE"    },
    { type:3, ar:"رومي",   name:"ROUMI",   ref:"MOSAIQUE · SKIP"    },
    { type:4, ar:"توريق",   name:"TAWRIK",   ref:"FEUILLAGE · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInMEKNES();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("MEKNES_IMPERIAL // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 11 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: MEKNES, MAROC", startX, startY);
