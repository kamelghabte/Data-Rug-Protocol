// DATA RUG PROTOCOL // 10 KÉNITRA
// SEBOU CORK MATRIX — Sebou / Maamora Code
...
// --------------------------------------------------
// 3) PALETTE KÉNITRA
// Vert forêt · Brun liège · Eau · Terre · Ivoire
// --------------------------------------------------
let palette = {
  foret:  "#1D6A39",
  liege:  "#7D6608",
...
function buildTextureLayer() {
  ...
  for (let i = 0; i < width * height * 0.0009; i++) {
    ...
    let y = random(height);
    let a = random(4, 12);
    textureLayer.fill(29, 106, 57, a);
    textureLayer.circle(x, y, random(0.3, 1.0));
  }

  for (let i = 0; i < 90; i++) {
    textureLayer.stroke(82, 190, 128, 5);
    let y = random(height);
...
function drawCells() {
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (cell.active) {
        drawKenitraMotif(x, y, cellW * 0.86, cellH * 0.86, cell.type, cell.col, cell.accent, cell.rot, energy);
...
function pickColor() {
  if (temp <= 16) {
    return random([palette.foret, palette.eau, palette.noir]);
  }
  if (temp >= 28) {
    return random([palette.liege, palette.terre, palette.mousse]);
  }
  return random([palette.foret, palette.liege, palette.eau, palette.mousse]);
}

function pickAccent(baseCol) {
  if (baseCol === palette.foret || baseCol === palette.mousse) return palette.liege;
  if (baseCol === palette.liege || baseCol === palette.terre) return palette.foret;
...
// --------------------------------------------------
// 10) MOTIFS KENITRA
// Foret et fleuve : formes pleines, 3 couches max
// --------------------------------------------------
function drawKenitraMotif(x, y, w, h, type, col, accent, rot, energy) {
  ...
  scale(breath);
  if (type === 0) {
    // FEUILLE -- ovale + tige, foret de Maamora
    fill(col);
    ellipse(0, -h * 0.08, w * 0.56, h * 0.72);
    fill(accent);
    rect(0, h * 0.28, w * 0.08, h * 0.3);
    fill(col);
    ellipse(0, -h * 0.08, w * 0.2, h * 0.28);
  } else if (type === 1) {
    // LIEGE -- carre arrondi dans carre, ecorce
    fill(col);
    rect(0, 0, w * 0.86, h * 0.86);
    fill(accent);
    rect(0, 0, w * 0.58, h * 0.58, w * 0.14);
    fill(col);
    rect(0, 0, w * 0.28, h * 0.28, w * 0.08);
  } else if (type === 2) {
    // SEBOU -- bande sinueuse, S du fleuve
    fill(col);
    rect(0, 0, w * 0.86, h * 0.86);
    fill(accent);
    ellipse(-w * 0.14, -h * 0.2, w * 0.48, h * 0.42);
    ellipse(w * 0.14, h * 0.2, w * 0.48, h * 0.42);
    fill(col);
    ellipse(-w * 0.14, -h * 0.2, w * 0.22, h * 0.18);
    ellipse(w * 0.14, h * 0.2, w * 0.22, h * 0.18);
  } else {
    // ANNEAU -- double anneau, bois coupe
    fill(col);
    ellipse(0, 0, w * 0.86, h * 0.86);
    fill(accent);
    ellipse(0, 0, w * 0.58, h * 0.58);
    fill(col);
    ellipse(0, 0, w * 0.34, h * 0.34);
    fill(accent);
...
function drawHUD() {
  ...
  fill(140);
  textSize(10);
  text("KÉNITRA, MAROC // INSTITUT FRANÇAIS DU MAROC", margin, ty);
  ...
  textSize(10);
  text(
    `34.2610N / 6.5802W  //  TEMP ${nf(temp, 1, 1)}°C  HUM ${humidity}%  WIND ${nf(wind, 1, 1)} m/s`,
    margin, ty + 56
  );
  ...
  fill(palette.ivoire);
  textSize(16);
  text("DATA RUG PROTOCOL // 10 KÉNITRA", width * 0.5, ty);

  fill(palette.mousse);
  textSize(9);
  text("SEBOU CORK MATRIX  —  Sebou / Maamora Code", width * 0.5, ty + 28);
  ...
  textAlign(LEFT, TOP);
  textSize(9);
  text("≤ 16°C  FORÊT", sx + 14, legendY);

  fill(palette.eau);
  rect(sx + sw, legendY, 10, 10);
  fill(130);
  text("17–27°C  FLEUVE", sx + sw + 14, legendY);

  fill(palette.liege);
  rect(sx + sw * 2, legendY, 10, 10);
  fill(130);
  text("≥ 28°C  LIÈGE CHAUD", sx + sw * 2 + 14, legendY);

  // Légende morphologique
  let morphY = legendY + 22;
  let motifNames = ["0·MAAMORA", "1·SEBOU", "2·LIÈGE", "3·ESTUAIRE"];
  let mstep = 88;
