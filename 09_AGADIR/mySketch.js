// ════════════════════════════════════════════════════════════════
//  AGADIR ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 09
// ════════════════════════════════════════════════════════════════
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInAGADIR()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInAGADIR() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInAGADIR() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // SHAMS — Carreau plein, soleil
    rect(x, y, w, h);
  } else if (type === 1) {
    // ARGAN — Bandes verticales, branches
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);
  } else if (type === 2) {
    // MAWJ — Triangle haut + rectangle bas, vague
    rect(x, y+h/2, w, h/2);
    fill(lerpColor(color(col), color(26, 26, 10), 0.3));
    triangle(x, y+h/2, x+w, y+h/2, cx, y);
  } else if (type === 3) {
    // BINAA — Losange, reconstruction
    quad(cx, y, x+w, cy, cx, y+h, x, cy);
  } else {
    // TAFUKT — Hélice, soleil berbère
    triangle(x, y, x+w, y, cx, cy);
    triangle(x+w, y+h, x, y+h, cx, cy);
    fill(lerpColor(color(col), color(26, 26, 10), 0.4));
...
function drawMuseumFrame() {
  ...
  fill(isNightInAGADIR() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInAGADIR() ? "أكادير بالليل" : "زليج أكادير", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInAGADIR() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("09", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"شمس",   name:"SHAMS",   ref:"SOLEIL · SNAKE"    },
    { type:1, ar:"أرگان",   name:"ARGAN",   ref:"ARBRE · DIAG"    },
    { type:2, ar:"موج",   name:"MAWJ",   ref:"VAGUE · REVERSE"    },
    { type:3, ar:"بناء",   name:"BINAA",   ref:"BETON · SKIP"    },
    { type:4, ar:"تافوكت",   name:"TAFUKT",   ref:"LUMIERE · SPIRAL"    },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInAGADIR();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("AGADIR_SOUSS // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 09 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: AGADIR, MAROC", startX, startY);
