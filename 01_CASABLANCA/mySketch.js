// ════════════════════════════════════════════════════════════════
//  CASABLANCA ZELLIGE  —  Weather Weaving
//  Institut Français du Maroc · Œuvre 01
// ════════════════════════════════════════════════════════════════
...
let palDay = [
  '#092E7A', // Bleu Majorelle
  '#1460C0', // Cobalt
  '#2A9CC8', // Turquoise ciel
  '#EFE8D8', // Blanc plâtre
  '#C8A65A', // Or ocre
  '#B83A0A', // Terracotta brûlée
  '#2C5840', // Vert menthe sombre
  '#4E2918', // Brun médina
];
...
let palNight = [
  '#04112E', // Bleu minuit
  '#0A2562', // Indigo profond
  '#1A3A8A', // Bleu roi
  '#2A6898', // Bleu orage
  '#C8A65A', // Or — la seule chaleur qui reste
  '#8A1A00', // Terracotta sombre
  '#E8E0D2', // Blanc plâtre — lunaire la nuit
  '#0A1828', // Quasi-noir
];

// Palette TEMPÊTE — quand le vent dépasse 12 m/s
let palStorm = [
  '#050D20', '#0A1E48', '#142E6A', '#1A4488',
  '#B8B0A8', '#889098', '#304858', '#081428',
];
...
function weaveOne(velocity, ci) {
  ...
  else if (isNightInCasablanca()) pal = palNight;
  else               pal = palDay;
  ...
  if (humidity > 75 && random() < map(humidity, 75, 100, 0, 0.7)) {
    col = lerpColor(color(col), color(isNightInCasablanca() ? '#E8E0D2' : '#EFE8D8'),
                    map(humidity, 75, 100, 0, 0.6)).toString('#rrggbb');
  }

  // MIDI fort → nuit graphique
  if (velocity > 108) col = isNightInCasablanca() ? '#04112E' : '#0A1828';
...
function drawZelligeMotif(x, y, w, h, type, col) {
  ...
  if (type === 0) {
    // ── QITT'A — Carreau plein, unité de base
    rect(x, y, w, h);

  } else if (type === 1) {
    // ── ASSABI — Bandes 1 sur 2
    let sw = w/3;
    for (let i = 0; i < 3; i += 2) rect(x + i*sw, y, sw, h);

  } else if (type === 2) {
    // ── RAQMA — Damier diagonal
    rect(x,     y,     w/2, h/2);
    rect(x+w/2, y+h/2, w/2, h/2);

  } else if (type === 3) {
    // ── MU'AYYAN — Losange plein
    quad(cx, y,  x+w, cy,  cx, y+h,  x, cy);

  } else {
    // ── DAWWAMA — Hélice 4 pales
    // Motif signature de Casablanca dans cette œuvre
    // 4 triangles, pales pair/impair teintées différemment
    // Référence aux propellers zellige des hammams de la médina
    triangle(x,   y,   x+w, y,   cx,  cy);  // pale haute
    triangle(x+w, y+h, x,   y+h, cx,  cy);  // pale basse

    // Pales latérales assombries — contraste qui donne la rotation
    fill(lerpColor(color(col), color(8, 12, 28), 0.45));
    triangle(x+w, y,   x+w, y+h, cx, cy);   // pale droite
    triangle(x,   y+h, x,   y,   cx, cy);   // pale gauche
  }
}
...
function drawMuseumFrame() {
  ...
  fill(isNightInCasablanca() ? '#C8A65A' : '#C8A65A');
  ...
  text(isNightInCasablanca() ? "الدار البيضاء بالليل" : "زليج كازابلانكا", width/2, margin/2);

  // Droite — n° + génération + mode
  textFont('Courier New'); textAlign(RIGHT, CENTER);
  textSize(7); textStyle(NORMAL); fill(50, 75, 125);
  text((isNightInCasablanca() ? "◆ NUIT · " : "◇ JOUR · ") + "GÉN " + nf(generation, 2), width - margin, margin/2 - 7);
  textSize(20); textStyle(BOLD); fill('#C8A65A');
  text("01", width - margin, margin/2 + 6);
}
...
function drawLegend() {
  ...
  let defs = [
    { type:0, ar:"قطعة",   name:"QITT'A",   ref:"BASE · SNAKE"    },
    { type:1, ar:"أصابع",  name:"ASSABI",   ref:"VENT · DIAG"     },
    { type:2, ar:"رقمة",   name:"RAQMA",    ref:"PLUIE · REVERSE" },
    { type:3, ar:"معيّن",  name:"MU'AYYAN", ref:"MIXTE · SKIP"    },
    { type:4, ar:"دوّامة", name:"DAWWAMA",  ref:"CHAUD · SPIRAL"  },
  ];

  // Couleurs représentatives selon heure
  let night = isNightInCasablanca();
  let rc = night
...
function drawCasaStats() {
  ...
  fill('#C8A65A');
  text("CASABLANCA_ZELLIGE // WEATHER_WEAVE", width/2 - 215, startY);

  textSize(7); textStyle(NORMAL); fill(42, 62, 105);
  text("Institut Français du Maroc  ·  Œuvre 01 / 12", width/2 - 138, startY + 20);

  textSize(8.5); textStyle(NORMAL); fill(130, 155, 195);
  text("LOC: CASABLANCA, MAROC", startX, startY);
